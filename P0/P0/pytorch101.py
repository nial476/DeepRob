...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.view(2, 3, 4)
    y = torch.transpose(y, 1, 0).flatten().view(3,8)
    #########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean = torch.sum(x, axis=0) / x.shape[0]
    var = torch.sum((x - mean) ** 2, axis=0) / (x.shape[0] - 1)
    std = torch.sqrt(var)
    y = (x - mean) / std
    ##########################################################################
    #                            END OF YOUR CODE                            #
